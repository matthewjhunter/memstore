...
| 7 | scoped, two releases | Retire `cmd/memstore-mcp` and the SQLite backend | Decided 2026-08-27, split the same day. **0.4.0 (tagged):** deprecation announced; nothing removed. **0.5.0:** the HTTP, MCP, and client tests move to Postgres (the fixture in `httpapi/handler_test.go` opens `sqlite :memory:` today, so the daemon's HTTP layer has been tested on a backend it never runs); the conformance suite keeps running against SQLite so the backend is not shipped untested; `setup` stops registering the stdio binary and the binary warns at startup; the Codex shim moves to `memstore hook`; `memstore import` gains a daemon target (today it only writes a SQLite file, so the documented export-then-import path does not exist yet). **0.6.0:** delete the binary, `Config.Embed`, `--read-only`, setup's stdio fallback, and the SQLite backend except the read-only opener `export --db` needs. **0.7.0:** drop the export reader. The split exists so tests that pass on SQLite and fail on Postgres can be checked against both while both still exist. |
