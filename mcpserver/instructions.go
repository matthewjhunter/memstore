// Package mcpserver's session instructions and the read-only decision behind
// them. They live here rather than in a command because they describe this
// package's tools, and because more than one transport now needs them: the
// stdio binary builds them at startup from a token query, and an in-process
// HTTP server will build them per request from the caller's identity (#173
