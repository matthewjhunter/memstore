 ...
 *      daemon renders them already fenced (/v1/context/hints/render); this hook
 *      never builds a hint block from raw text.
 *
...
const [recallResult, hintsResult] = await Promise.allSettled([
  ...
  (sessionId || cwd) ? fetchHints(sessionId, cwd) : Promise.resolve(NO_HINTS),
]);
...
async function fetchHints(sessionId, cwd) {
  ...
  params.set('limit', String(MAX_HINTS));
  const resp = await fetch(
    `${MEMSTORED_URL}/v1/context/hints/render?${params}`,
    { headers: authHeaders(), signal: AbortSignal.timeout(2000) },
  );
  // A daemon older than the render endpoint only has the raw list. Building a
  // block from that here would inject model-written text unfenced (#219), so
  // skip hints until the daemon is upgraded.
  if (resp.status === 404) {
    warnOnce('memstore-prompt: memstored has no /v1/context/hints/render; hints skipped until it is upgraded');
    return NO_HINTS;
  }
  if (!resp.ok) {
    reportRefusal('memstore-prompt', resp);
