...
Every store-backed suite opens its store through `internal/teststore`, which creates a private PostgreSQL database per test on the server `MEMSTORE_TEST_PG` names and skips when it is unset -- so an unset variable means most of the suite did not run. A throwaway server: `docker run -d --rm --name pg -e POSTGRES_USER=test -e POSTGRES_PASSWORD=test -e POSTGRES_DB=test -p 55432:5432 pgvector/pgvector:pg17`, then `MEMSTORE_TEST_PG='postgres://test:test@localhost:55432/test?sslmode=disable' GOWORK=off go test ./...`. A test never connects to `MEMSTORE_TEST_PG` itself: it takes a private database from `internal/testpg` (`testpg.Pool`/`testpg.DSN`, or the `testPool`/`newTestStoreOn` helpers in `pgstore`), and a test needing raw SQL alongside a store passes that one pool to both. The pgstore suite used to share the named database and rebuild a clean slate by dropping a list of tables; the list never covered `memstore_fact_chunks`, so a store left at another vector dimension -- by a real daemon pointed at the test server, or a crashed run -- failed unrelated tests with "expected 768 dimensions, not 4", and only ever on a developer machine, since CI's container is new each run. Tests take a `teststore.Store` and compare JSON by value rather than substring (jsonb reflows it). Mock embedders produce `teststore.VecDim` (4) wide vectors; the schema is `vector(4)`, so a test vector of any other width is rejected by Postgres.
...
- The fact chunk ANN index (`idx_memstore_fact_chunk_ann_<dim>`, `pgstore/factchunkann.go`) is an HNSW index on the expression `embedding::vector(<dim>)`, because the live columns carry no dimension and pgvector cannot index an unconstrained column. Both clear paths drop it through `dropANNIndexesSQL`: vectors from a new model could not be inserted under an index cast to the old dimension, so a new clear path needs it too. The ANN query must order by the same cast as the index expression, or the planner silently falls back to a scan. The ANN tests open their store with no vector dimension, as production does -- with `vector(N)` columns the planner strips the cast and the index is never used. `TestANNQueryUsesIndex` pins the plan, and `TestSearchVectorANNMatchesExact` pins that the index changes no result.
